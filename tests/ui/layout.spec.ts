...
test ('Add new layout', async ({page}) => {
    await page.goto('https://keycloak.common.nuclent.com/auth/realms/ncore_new_org/protocol/openid-connect/auth?client_id=ncore_new_org&response_type=code&state=ux1SWwrTvj1vrXHE35ZOc&redirect_uri=https%3A%2F%2Fnew_org.nflow.dev.nuclent.com%2Fauth%2Fsso%2Fcallback%3Fpath%3D%252F&scope=openid');
        ...
        await test.step('Open popup', async () => {
            ...
            await newTab.locator('a:nth-child(3) > .mantine-Indicator-root > .mantine-UnstyledButton-root').click();
            await newTab.getByRole('button', { name: 'New Layout' }).click();
            await newTab.getByRole('textbox', { name: 'Display name' }).fill('Layout Auto');
            await newTab.getByRole('textbox', { name: 'Display name' }).press('Tab');
            await newTab.getByRole('searchbox', { name: 'Layout Type' }).click();
            await newTab.getByText('App').nth(0).click();
            await newTab.getByRole('textbox', { name: 'Description' }).click();
            await newTab.getByRole('textbox', { name: 'Description' }).fill('Layout is added by Auto');
...
test ('Edit Layout', async ({page}) => {
    ...
    await test.step('Open popup', async () => {
        ...
        await expect(newTab).toHaveURL('https://new_org.nflow.dev.nuclent.com/setup/app');
        await newTab.locator('a:nth-child(3) > .mantine-Indicator-root > .mantine-UnstyledButton-root').click();
        await newTab.getByText('Layout Auto').click();
        await newTab.locator('.mantine-Group-root > button').first().click();
        await newTab.getByRole('textbox', { name: 'Display name' }).fill('Layout Auto');
        await newTab.getByRole('textbox', { name: 'Description' }).click();
        await newTab.getByRole('textbox', { name: 'Description' }).fill('Layout is added by Auto');
        await newTab.locator('.mantine-1n7zxp').click();
        await newTab.locator('div').filter({ hasText: /^Video call$/ }).first().click();
        await newTab.getByRole('searchbox', { name: 'Tags' }).click();
        await newTab.getByRole('button', { name: 'Next' }).click();
